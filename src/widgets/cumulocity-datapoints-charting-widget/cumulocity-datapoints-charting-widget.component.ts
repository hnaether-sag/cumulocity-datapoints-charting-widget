...
export class CumulocityDatapointsChartingWidget implements OnDestroy, OnInit {
        ...
        if (Object.prototype.hasOwnProperty.call(config.series, seriesName)) {
          ...
          if (!seriesConfig.isParent) {
            ...
            for (let index = 0; index < seriesConfig.idList.length; index++) {
              ...
              const splitSeriesId = seriesConfig.idList[index].split('.');
            ...
            if (options.targetGraphType == 'pie' || options.targetGraphType == 'doughnut') {
              ...
              this.createPieChart(
                seriesConfig.idList.length > 1 ? seriesConfig.idList[0] : seriesName,
                localChartData,
                options
              );
            } else {
  ...
  async handleRealtime(dataObject: DataObject): Promise<void> {
    const measurementDate = new Date(dataObject.data.data.data.time);
    const config = this.widgetHelper.getChartConfig();
    ...
    if (has(dataObject.data.data.data, dataObject.options.fragment)) {
      const frag = get(dataObject.data.data.data, dataObject.options.fragment);
      if (has(frag, dataObject.options.series)) {
        const ser = get(frag, dataObject.options.series);
        ...
        const newPointBucket = this.measurementHelper.categorize(dataObject.options, datum);
        let lastPointBucket = '';
        ...
        if (config.getChartType() == 'pie' || config.getChartType() == 'doughnut') {
          ...
          if (config.aggregation == 0) {
            ...
            //By Value buckets
            const vals = this.seriesData[dataObject.key].valtimes.map((val) => <number>val.y);
            const hist = this.measurementHelper.calculateHistogram(
              vals,
              config.maxBucket,
              config.minBucket,
              config.sizeBuckets,
          ...
          if (dataObject.options.avgPeriod > 0) {
            //just the values
            const source = this.seriesData[dataObject.key].valtimes
            ...
            const avper = dataObject.options.avgPeriod > source.length ? source.length : dataObject.options.avgPeriod;
            const a = boll(source, avper, 2);
        ...
        if (this.chartElement) {
          ...
          //
          if (config.getChartType() === 'line' || config.getChartType() === 'spline') {
          ...
          if (config.getChartType() === 'pie' || config.getChartType() === 'doughnut') {
            ...
            if (config.aggregation == 0) {
              const aggUnit = config.rangeUnits[config.aggTimeFormatType].text;
  ...
  private updateSeriesData(dataObject: DataObject, newPointBucket: string, lastPointBucket: string, datum: ChartPoint) {
    ...
    if (dataObject.options.groupby && newPointBucket === lastPointBucket) {
      ...
      if (config.getChartType() == 'horizontalBar') {
        ...
        const v = valtimes[valtimes.length - 1].x as number;
        valtimes[valtimes.length - 1].x = (<number>datum.x + v * (valCount - 1)) / valCount;
        valtimes[valtimes.length - 1].x = parseFloat(
          (<number>valtimes[valtimes.length - 1].x).toFixed(dataObject.options.numdp)
        );
      } else {
        const { valtimes, valCount } = this.seriesData[dataObject.key];
        const v = valtimes[valtimes.length - 1].y as number;
        valtimes[valtimes.length - 1].y = (<number>datum.y + v * (valCount - 1)) / valCount;
        valtimes[valtimes.length - 1].y = parseFloat(
          (<number>valtimes[valtimes.length - 1].y).toFixed(dataObject.options.numdp)
        );
      }
    } else {
      this.seriesData[dataObject.key].valCount = 1;
  ...
  private updateGroupData(options: MeasurementOptions, datum: ChartPoint) {
    ...
    if (options.group !== 'default' && config.groupbyGroup) {
      const lastElement: ChartPoint =
        this.seriesData[options.group].valtimes[this.seriesData[options.group].valtimes.length - 1];
      const nextTime = moment(datum.x).format(options.labelDateFormat);
      const lastTime = lastElement ? moment(lastElement.x).format(options.labelDateFormat) : undefined;
      ...
      if (lastTime !== nextTime) {
        ...
        if (!config.groupCumulative) {
          ...
          if (config.getChartType() == 'horizontalBar') {
            const v = valtimes[valtimes.length - 1].x;
            valtimes[valtimes.length - 1].x = (<number>datum.x + <number>v * (valCount - 1)) / valCount;
            valtimes[valtimes.length - 1].x = parseFloat(
              (<number>valtimes[valtimes.length - 1].x).toFixed(options.numdp)
            );
          } else {
            const v = valtimes[valtimes.length - 1].y;
            valtimes[valtimes.length - 1].y = (<number>datum.y + <number>v * (valCount - 1)) / valCount;
            valtimes[valtimes.length - 1].y = parseFloat(
  ...
  verifyConfig(): boolean {
    ...
    if (config.enabled) {
      if (
        ...
        const checks = this.checkMultivariateChart();
  ...
  private async retrieveAndPlotMultivariateChart(localChartData: ChartDataSets[]) {
    const seriesList: { [id: string]: string } = {};
    let assigned = 0;
    const config = this.widgetHelper.getChartConfig();
    //
  ...
  private checkMultivariateChart(): {
    ...
    const rval = { series: 0, x: false, y: false, r: false };
    //
    // Get the data - there will be 1-3 series that will get
    // compressed into a single with x,y,r values
    //
    const config = this.widgetHelper.getChartConfig();
    for (const key of Object.keys(config.series)) {
  ...
  private createMultivariateChart(seriesList: { [id: string]: string }, localChartData: ChartDataSets[]) {
    ...
    for (let index = 0; index < this.seriesData[seriesList['x']].valtimes.length; index++) {
      const xval = this.seriesData[seriesList['x']].valtimes[index];
      const yval = this.seriesData[seriesList['y']].valtimes.filter((val) => {
        return (
  ...
  private async getBaseMeasurements(
    ...
    const config = this.widgetHelper.getChartConfig();
    const measurementLimit = config.rangeType > 0 ? 0 : config.rangeValue;

    let unitIndex = config.timeFormatType;
    if (config.getChartType() == 'pie' || (config.getChartType() == 'doughnut' && config.aggregation == 0)) {
      unitIndex = config.aggTimeFormatType;
    }

    const aggUnit = config.rangeUnits[unitIndex].text;
    let aggFormat = get(config.rangeDisplay, aggUnit);
  ...
  private createPieChart(key: string, localChartData: ChartDataSets[], options: MeasurementOptions) {
    ...
    const thisSeries: ChartDataSets = {
      data: [],
      backgroundColor: [...colorList, ...colorList, ...colorList, ...colorList, ...colorList, ...colorList], //repeated so we can avoid having to update
    };
        ...
        if (Object.prototype.hasOwnProperty.call(config.series, seriesName)) {
          ...
          if (!seriesConfig.isParent) {
            ...
            for (let index = 0; index < seriesConfig.idList.length; index++) {
              ...
              let deviceId = this.widgetHelper.getDeviceTarget();
              const splitSeriesId = seriesId.split('.');
              if (deviceId === undefined) {
                deviceId = splitSeriesId[0];
              }
  ...
  private getDateRange(): { from: Date; to: Date } {
    ...
    const timeUnitVal: number = config.rangeUnits[config.rangeType ? config.rangeType : 4].id;

    const from = new Date(to - config.rangeValue * timeUnitVal * 1000);
  ...
  private setAxes() {
    ...
    if (config.getChartType() === 'horizontalBar') {
      //swapped x & y
      const timeUnitType = config.rangeUnits[config.rangeType ? config.rangeType : 2].text as Chart.TimeUnit;
      ...
      const dp = config.numdp ? config.numdp : 2;
      ...
      if (config.multivariateplot) {
        ...
        const timeUnitType = config.rangeUnits[config.rangeType ? config.rangeType : 1].text as Chart.TimeUnit;
        this.chartOptions.scales.xAxes.push({
          display: config.showx,
          stacked: config.stackSeries,
          type: 'time',
