...
export class CumulocityDatapointsChartingWidgetConfig
  implements OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    if (this.widgetHelper.getDeviceTarget()) {
      this.getDeviceDetail(this.widgetHelper.getDeviceTarget())
        .then((result) => {
          let v: RawListItem = {
            id: result.data.id,
            text: result.data.name,
            isGroup: result.data.hasOwnProperty('c8y_IsDeviceGroup') || result.data.hasOwnProperty('c8y_IsGroup'),
          };
        ...
        .catch((error) =>
          this.alertService.danger(
            `There was an issue getting device details, please refresh the page.`,
            error
          )
        );
    } else {
  ...
  async fetchSeries(id): Promise<string[]> {
    const url = `/inventory/managedObjects/${id}/supportedSeries`;
    const resp: IFetchResponse = await this.fetchclient.fetch(url);
    if (resp.status === 200) {
  ...
  async getDevicesForGroup(id: string): Promise<IManagedObject[]> {
    ...
    //get the additions
    let result: IResultList<IManagedObject> =
      await this.inventory.childAdditionsList(id, childFilter);
  ...
  async getSupportedSeries(devices: RawListItem[]): Promise<RawListItem[]> {
    ...
    for (const device of devices) {
      ...
      if (device.isGroup) {
        ...
        let actualDevices = await this.getDevicesForGroup(device.id);

        for (let index = 0; index < actualDevices.length; index++) {
          const device = actualDevices[index];
          let current: RawListItem[] = (await this.fetchSeries(device.id)).map(
            (m) => {
              return {
                id: device.id + "." + m,
                text: `${m}(${device.text}/${device.name})`,
                isGroup: true,
                groupname: device.text,
              };
            }
          );
          local = [...local, ...current];
        ...
        try {
          const series = await this.fetchSeries(device.id);
          const current = series.map((m) => {
            return {
              id: device.id + "." + m,
              text: `${m}(${device.text})`,
              isGroup: false,
  ...
  async updateSelectedMeasurements() {
    ...
    this.widgetHelper.getWidgetConfig().selectedMeasurements.forEach((v, i) => {
      ...
      if (
        v.isGroup &&
        !(v.groupname in this.widgetHelper.getChartConfig().series)
      ) {
        ...
        this.widgetHelper
          .getChartConfig()
          .series[v.groupname].idList.push(v.id);
      }
    });
  }
  ...
  async clearCache() {
    ...
    await deleteDB(dbName, {
      blocked: () => console.log(`Waiting to Removing DB ${dbName}`),
    });
  }
    ...
    if (chart && conf.selectedDevices && conf.selectedDevices.length > 0) {
      ...
      if (conf.selectedMeasurements && conf.selectedMeasurements.length > 0) {
        for (let index = 0; index < conf.selectedMeasurements.length; index++) {
          if (
            checklist.has(conf.selectedMeasurements[index].id.split(".")[0])
          ) {
      ...
      this.supportedSeries.next(
        await this.getSupportedSeries(conf.selectedDevices)
      );
    }
