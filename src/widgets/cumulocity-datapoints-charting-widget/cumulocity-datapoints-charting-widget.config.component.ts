...
export class CumulocityDatapointsChartingWidgetConfig implements OnInit, OnDestroy {
  ...
  updateSelectedMeasurements(): void {
    ...
    selectedMeasurements.forEach((v, i) => {
      this.widgetHelper
        .getChartConfig()
        .addSeries([v.id.toString()], v.text, config.colorList[i], config.avgColorList[i], v.groupname);
  ...
  updateConfig(): void {
    ...
    //Some charts need certain defaults
    if ((chartType === 'scatter' || chartType === 'bubble') && config.showPoints == 0) {
      config.showPoints = 4;
    }

    //Bar and horizontalBar should be time based
    if (chartType === 'bar' || chartType === 'horizontalBar' || chartType === 'pie' || chartType === 'doughnut') {
