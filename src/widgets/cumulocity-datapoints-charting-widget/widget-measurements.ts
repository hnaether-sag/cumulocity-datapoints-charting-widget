...
export class MeasurementHelper {
  ...
  public async getMeasurements(
    ...
    if (data.length > 0) {
      ...
      console.log(
        `retrieved stored data for ${key} = ${fromDateInRange} => ${toDateInRange}`
      );
    } else {
    ...
    if (
      ...
      if (
        ...
        let newData = await this.getDataFromC8y(
          filter,
          measurementService,
          data,
          maxMeasurements
      ...
      let filter = options.filter();
      //get the first page
      data = await this.getDataFromC8y(
        filter,
        measurementService,
        data,
        maxMeasurements
    ...
    if (startIndex >= 0) {
      ...
      while (
        ...
        moment(rangeStart).isBefore(dateFrom)
      ) {
  ...
  public async createAggregate(
    ...
    if (!sumData) {
      for (let index = 0; index < rawData.vl.length; index++) {
        ...
        rawData.vl[index].y = parseFloat(
          (point.y / measurements.length).toFixed(options.numdp)
        );
      }
    }
  ...
  private retrieveData(
    ...
    let d = data.reduce((newArr, row) => {
      ...
      if (row.hasOwnProperty(options.fragment)) {
        const frag = row[options.fragment];
        if (frag.hasOwnProperty(options.series)) {
          const ser = frag[options.series];
      ...
      if (options.groupby === true) {
        ...
        if (newArr.lastBucket != lst) {
          ...
          if (options.targetGraphType == "horizontalBar") {
            newArr.vl[newArr.vl.length - 1].x =
              newArr.vl[newArr.vl.length - 1].x + v.x;
          } else {
            newArr.vl[newArr.vl.length - 1].y =
              newArr.vl[newArr.vl.length - 1].y + v.y;
          }
    ...
    if (options.groupby === true) {
      if (!options.cumulative) {
        result.vl = d.vl.map((val, index) => {
          if (options.targetGraphType == "horizontalBar") {
            val.x = parseFloat(
              (val.x / d.vlCounts[index]).toFixed(options.numdp)
            );
          } else {
            val.y = parseFloat(
              (val.y / d.vlCounts[index]).toFixed(options.numdp)
            );
          }
          return val;
        });
    ...
    if (
      ...
      if (options.timeBucket) {
        ...
        rawData.vl.map((val) => {
          ...
          if (result.hasOwnProperty(mapped)) {
  ...
  calculateHistogram(
    ...
    for (let i = 0; i < numBins; i++) {
      bins.push(0);
      let upper = Math.min(minBucket + (i + 1) * binSize, maxBucket).toFixed(
        dp
      );
