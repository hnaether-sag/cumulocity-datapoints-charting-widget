...
export class MeasurementHelper {
  ...
  public async getMeasurements(
    ...
    const key = `${chartID}-${deviceId}.${fragment}.${series}`;
    ...
    if (
      data.length &&
      moment(dateFrom).isSameOrAfter(fromDateInRange) &&
      moment(dateFrom).isSameOrBefore(toDateInRange)
    ) {
      ...
      if (moment(dateTo).isSameOrBefore(toDateInRange) && moment(dateTo).isSameOrAfter(fromDateInRange)) {
        ...
        const newData = await this.getDataFromC8y(filter, measurementService, data, maxMeasurements);
        data = [...data, ...newData];
      }
    } else if (
      data.length > 0 &&
      moment(dateTo).isSameOrBefore(toDateInRange) &&
      moment(dateTo).isSameOrAfter(fromDateInRange)
    ) {
  ...
  public async createAggregate(
    ...
    const upper: Chart.ChartPoint[] = [];
    const lower: Chart.ChartPoint[] = [];
    const aggseries: Chart.ChartPoint[] = [];
    ...
    for (let index = 0; index < measurements.length; index++) {
      ...
      if (rawData.vl.length == 0) {
        ...
        //add the values
        const theLength = Math.min(rawData.vl.length, raw.valtimes.length);
    ...
    const measurementList: MeasurementList = new MeasurementList(
      options,
      upper,
      aggseries,
      lower,
  ...
  private processData(data: IMeasurement[], options: MeasurementOptions): MeasurementList {
    //get the data.
    const rawData: RawData = this.retrieveData(data, options);
  ...
  private retrieveData(data: IMeasurement[], options: MeasurementOptions): RawData {
    const d = data.reduce((newArr, row) => {
      //default
      const measurementDate = new Date(row.time);
      ...
      if (has(row, options.fragment)) {
        const frag = get(row, options.fragment);
        if (has(frag, options.series)) {
          const ser = get(frag, options.series);
      ...
      if (options.groupby === true) {
        const lst = this.categorize(options, v);
  ...
  private createAggregateSeries(
    ...
    rawData: RawData,
    upper: Chart.ChartPoint[],
    aggseries: Chart.ChartPoint[],
    lower: Chart.ChartPoint[]
  ) {
    if (options.targetGraphType == 'line') {
      if (options.avgPeriod && options.avgPeriod > 0) {
        //just the values
        const source = rawData.vl.map((val) => val.y);
        if (!source.length) {
          return;
        }
        //average and bollinger bands
        const avper = options.avgPeriod > source.length ? source.length : options.avgPeriod;
    ...
    if (
      ...
      if (options.timeBucket) {
        ...
        rawData.vl.map((val) => {
          ...
          // simple 1 dim array
          const mapped = this.categorize(options, val);
        ...
        //values buckets
        const vals = rawData.vl.map((val) => val.y as number);
        const hist = this.calculateHistogram(
          vals,
          options.mxBuckets,
          options.mnBuckets,
          options.sizeBuckets,
  ...
  calculateHistogram(
    ...
    const dataCopy = arr.sort((a, b) => a - b);

    bins.push(0); //lower catch all
    binLabels.push(`< ${minBucket}`);

    const numBins = Math.floor((maxBucket - minBucket) / binSize) + 1;
    //Initialize to 0 and labels
    for (let i = 0; i < numBins; i++) {
      bins.push(0);
      const upper = Math.min(minBucket + (i + 1) * binSize, maxBucket).toFixed(dp);
